...
export default function PaymentTermsPage() {
  ...
  const loadPaymentTerms = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof PaymentTermsApiError
          ? err.message
  ...
  const handleArchivePaymentTerm = useCallback(
    async (paymentTerm: PaymentTermDto) => {
      if (
        !confirm(
          `Archive payment term "${paymentTerm.name}"? This will disable it from future use.`,
        )
      ) {
        return;
      }
  ...
  const columns: TableColumn<PaymentTermDto>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (_, record) => {
          if (
            !record.early_payment_discount_percent ||
            !record.early_payment_discount_days
          ) {
            return (
          ...
          return (
            <div className="text-sm text-gray-600 dark:text-gray-300">
              {record.early_payment_discount_percent}% within{" "}
              {record.early_payment_discount_days} days
            </div>
          );
        },
      },
      {
        ...
        render: (_, record) => (
          <StatusBadge
            ...
            status={record.archived_at ? "Archived" : "Active"}
          />
        ),
      },
      {
            ...
            actions={[
              {
                ...
                onClick: () =>
                  router.push(`/master-data/payment-terms/${record.id}/edit`),
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Manage payment terms for customers and suppliers.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          <Button
            ...
            onClick={() => router.push("/master-data/payment-terms/create")}
          >
            New Payment Term
          </Button>
        </div>
