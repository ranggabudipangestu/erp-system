...
export default function ChartOfAccountsPage() {
  ...
  const handleArchive = useCallback(
    async (account: ChartOfAccountTreeDto) => {
      if (
        !confirm(
          `Archive account "${account.name}" (${account.code})? This account will no longer be available.`,
        )
      )
        return;
      try {
        setProcessing(true);
        setError(null);
        await coaApi.archiveAccount(account.id);
        setSuccessMessage(`Account "${account.name}" archived successfully.`);
        await loadTree();
      } catch (err) {
        const message =
